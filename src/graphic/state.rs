...
impl State {
    pub(crate) async fn new(window: Window, path: impl AsRef<std::path::Path>) -> Self {
        // steal window size
        ...
        let (cell_state, cells_bind_group_layout) = CellState::new_from_file(&device, path);
...
pub async fn run(path: impl AsRef<std::path::Path>) {
    env_logger::init();
    ...
    let mut state = State::new(window, path).await;
...
fn window_events(state: &mut State, control_flow: &mut ControlFlow, event: &WindowEvent<'_>) {
    match event {
